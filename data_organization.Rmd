...
option_chain_aggregator = function(input_option_chain, min_strike_ratio=NA, max_strike_ratio=NA,min_avg_days_to_expiry=NA,max_avg_days_to_expiry=NA){
  ...
  input_option_chain$mean_days_to_expiry = mean(c(input_option_chain$days_to_expiry,input_option_chain$business_days_to_expiry))
  expiry_dates = unique(input_option_chain$expiration)
  ...
  if(is.na(min_avg_days_to_expiry) & is.na(max_avg_days_to_expiry)){
    ...
    input_option_chain = input_option_chain[input_option_chain$option_ratio >= min_avg_days_to_expiry,]
    input_expiry_range = paste0(min_avg_days_to_expiry," < x")
  }else if(!is.na(min_avg_days_to_expiry) & !is.na(max_avg_days_to_expiry)){
    input_option_chain = input_option_chain[input_option_chain$option_ratio >= min_avg_days_to_expiry,]
    input_option_chain = input_option_chain[input_option_chain$option_ratio < max_avg_days_to_expiry,]
  ...
  temp_results <- list("option_underlying_ticker"=input_ticker,
                       ...
                       "option_strike_max"=min_strike_ratio*input_avg_underlying,
                       "option_strike_range"=input_strike_range,
                       "option_expiry_range"=input_expiry_range,
                       "option_style"=input_style,
                       "option_call_total_volume"=sum(sub_option_chain$volume[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_call_total_open_interest"=sum(sub_option_chain$open_interest[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_call_mean_delta"=mean(sub_option_chain$delta[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_call_mean_gamma"=mean(sub_option_chain$gamma[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_call_mean_theta"=mean(sub_option_chain$theta[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_call_mean_vega"=mean(sub_option_chain$vega[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_call_mean_implied_volatility"=mean(sub_option_chain$implied_volatility[sub_option_chain$type == "call"],na.rm = TRUE),
                       "option_put_total_volume"=sum(sub_option_chain$volume[sub_option_chain$type == "put"],na.rm = TRUE),
                       "option_put_total_open_interest"=sum(sub_option_chain$open_interest[sub_option_chain$type == "put"],na.rm = TRUE),
                       "option_put_mean_delta"=mean(sub_option_chain$delta[sub_option_chain$type == "put"],na.rm = TRUE),
                       "option_put_mean_gamma"=mean(sub_option_chain$gamma[sub_option_chain$type == "put"],na.rm = TRUE),
                       "option_put_mean_theta"=mean(sub_option_chain$theta[sub_option_chain$type == "put"],na.rm = TRUE),
                       "option_put_mean_vega"=mean(sub_option_chain$vega[sub_option_chain$type == "put"],na.rm = TRUE),
                       "option_put_mean_implied_volatility"=mean(sub_option_chain$implied_volatility[sub_option_chain$type == "put"],na.rm = TRUE))
...
# Load options data
dfOptions = read.delim("2013-01-02options.cvs",header=TRUE,sep=",")

# Displaying the first few rows of the dataframe
head(dfOptions)

# Load stock data
dfStocks = read.delim("2013-01-02stocks.cvs",header=TRUE,sep=",")
head(dfStocks)
...
for(temp_ticker_i in 1:length(sp500_tickers)){
  ...
  if(temp_ticker %in% filteredOptions$underlying){
    ...
    test_result = option_chain_aggregator(test_chain,max_strike_ratio = 0.5)
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 0.75,min_strike_ratio = 0.5))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 0.9,min_strike_ratio = 0.75))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 0.95,min_strike_ratio = 0.9))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 1,min_strike_ratio = 0.95))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 1.1,min_strike_ratio = 1.05))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 1.25,min_strike_ratio = 1.1))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,max_strike_ratio = 1.5,min_strike_ratio = 1.25))
    test_result = rbind(test_result,option_chain_aggregator(test_chain,min_strike_ratio = 1.5))

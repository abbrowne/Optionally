...
```{r testing more optimized implementations}
...
batch_i <- 1
timestamp()
while(batch_i*24 < length(date_list)){
  sub_file_paths <- file_paths[(1+(24*(batch_i-1))):(24*(batch_i))]
  cl <- makeCluster(num_cores)
...
sub_file_paths <- file_paths[(1+(24*(batch_i-1))):(length(date_list))]
cl <- makeCluster(num_cores)
clusterExport(cl, c("sub_file_paths", "option_chain_aggregatorV3", "process_fileV2"))
clusterEvalQ(cl, {
  library(data.table)

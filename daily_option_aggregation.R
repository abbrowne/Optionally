...
if(length(test_option_files) > 0){
  ...
  temp_dupCheck <- paste0(full_aggregate$option_underlying_ticker,"_",full_aggregate$option_quote_date)
  full_aggregate <- full_aggregate[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(EMA5_df$option_underlying_ticker,"_",EMA5_df$option_quote_date)
  EMA5_df <- EMA5_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(EMA10_df$option_underlying_ticker,"_",EMA10_df$option_quote_date)
  EMA10_df <- EMA10_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(EMA20_df$option_underlying_ticker,"_",EMA20_df$option_quote_date)
  EMA20_df <- EMA20_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(EMA50_df$option_underlying_ticker,"_",EMA50_df$option_quote_date)
  EMA50_df <- EMA50_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(SMA5_df$option_underlying_ticker,"_",SMA5_df$option_quote_date)
  SMA5_df <- SMA5_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(SMA10_df$option_underlying_ticker,"_",SMA10_df$option_quote_date)
  SMA10_df <- SMA10_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(SMA20_df$option_underlying_ticker,"_",SMA20_df$option_quote_date)
  SMA20_df <- SMA20_df[!duplicated(temp_dupCheck),]
  ...
  temp_dupCheck <- paste0(SMA50_df$option_underlying_ticker,"_",SMA50_df$option_quote_date)
  SMA50_df <- SMA50_df[!duplicated(temp_dupCheck),]
  
  if(length(all_dates) < 50){
    alt_start_date <- start_dates[start_dates != start_dates[1]][1]
    alt_end_date <- end_dates[start_dates != start_dates[1]][1]
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("full_option_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    ...
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("EMA5_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    EMA5_df <- rbind(temp_aggregate,EMA5_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("EMA10_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    EMA10_df <- rbind(temp_aggregate,EMA10_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("EMA20_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    EMA20_df <- rbind(temp_aggregate,EMA20_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("EMA50_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    EMA50_df <- rbind(temp_aggregate,EMA50_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("SMA5_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    SMA5_df <- rbind(temp_aggregate,SMA5_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("SMA10_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    SMA10_df <- rbind(temp_aggregate,SMA10_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("SMA20_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
    SMA20_df <- rbind(temp_aggregate,SMA20_df)
    
    temp_aggregate <- readRDS(paste0(main_dir,"revised_derived_aggregates/",
                                     test_files[grepl("SMA50_aggregate_",test_files) & grepl(alt_end_date,test_files)]))
    temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= temp_start_date,]
  ...
  for(temp_file_i in 1:length(aggregate_files)){
    ...
    if(length(new_date) > 1){
      ...
      if(length(test_dates) == 100){
        ...
        dates_to_keep <- test_dates[(length(test_dates)-49):length(test_dates)]
...
temp_first_date <- last_split_date + 1
temp_last_date <- Sys.Date()
...
all_splits <- all_splits[all_splits$date > temp_first_date,]
if(nrow(all_splits) > 0){
  if(is.null(current_splits)){
    saveRDS(all_splits,"E:/Market_Data/SP500_splits_current_working.RDS")
  }else{
    current_splits <- rbind(current_splits,all_splits)
    saveRDS(current_splits,"E:/Market_Data/SP500_splits_current_working.RDS")
  }
}
...
safe_execute_yahoo_prices_alt <- function(input_ticker,input_start_date,input_end_date){
  temp_results <- NULL
  max_attempts <- 5
  attempt <- 1
  while(is.null(temp_results) && attempt <= max_attempts){
    ...
    temp_results <- tryCatch(
      {
        # Attempt to execute the function
        as.data.frame(getSymbols(input_ticker, src = "alphavantage",api.key="WET93PLEDSXLAMHU", 
                                 from = input_start_date, to = input_end_date, auto.assign = FALSE))
      },
      error = function(e) {
        # Print the error message
        message("An error occurred: ", e$message)
...
for(temp_ticker_i in 1:length(SP500_tickers)){
  ...
  if(is.null(current_results)){
    ...
    start_date <- max(current_results$Date) + 1
  }
  end_date <- Sys.Date()
...
full_results <- readRDS("E:/Market_Data/SP500_3yr_price_history_current_working.RDS")
...
saveRDS(full_results,file="E:/Market_Data/SP500_3yr_price_history_with_EMAs_current_working.RDS")

full_results <- readRDS("E:/Market_Data/SP500_3yr_price_history_with_EMAs_current_working.RDS")

###

##Derive outcomes by checking if EMAX is greater than threshold for price X days prior
riskyAggregation <- function(all_tickers, full_results){
  new_columns <- NULL
  for(temp_ticker_i in 1:length(all_tickers)){
    temp_ticker <- all_tickers[temp_ticker_i]
    sub_results <- full_results[full_results$Ticker == temp_ticker,]
  ...
  outcome_results <- as.data.frame(new_columns)
...
max_attempts <- 5
attempt <- 1
while(is.null(new_results) && attempt <= max_attempts){
  cat("Attempt", attempt, "...\n")
  new_results <- tryCatch({
    riskyAggregation(all_tickers, full_results)
  }, error = function(e) {
    # Handle error by printing it and returning NULL
    cat("An error occurred: ", e$message, "\n")
    NULL  # Ensure result stays NULL if an error occurs
  })
  attempt <- attempt + 1
}
if(!is.null(new_results)){
  attempt <- attempt - 1
  cat("Operation succeeded after", attempt, "attempts.\n")
}else{
  cat("Operation failed after", max_attempts, "attempts.\n")
}


rownames(new_results) <- new_results$temp_rownames
new_results <- new_results[,colnames(new_results) != "temp_rownames"]
for(temp_counter_i in 1:50){
  new_results[,paste0("EMA5ahead",temp_counter_i)] <- as.numeric(new_results[,paste0("EMA5ahead",temp_counter_i)])
  new_results[,paste0("avgOpenClose_ahead",temp_counter_i)] <- as.numeric(new_results[,paste0("avgOpenClose_ahead",temp_counter_i)])
}

full_results <- full_results[rownames(full_results) %in% rownames(new_results),]
full_results <- cbind(full_results,new_results[rownames(full_results),])
...
days_to_test <- c(5,10,15,20,25,50)
pcts_to_test <- c(01,02,03,04,05,06,07,08,09,10,15,20,25,50)
for(temp_day_i in 1:length(days_to_test)){
  temp_day <- days_to_test[temp_day_i]
  for(temp_pct_i in 1:length(pcts_to_test)){
    temp_pct <- str_pad(pcts_to_test[temp_pct_i], width = 2, side = "left", pad = "0")
    temp_pctVal <- as.numeric(paste0("1.",temp_pct))
    temp_colname <- paste0("outcomes_",temp_day,"d",temp_pct,"pctEMA5above")
    temp_EMACol <- paste0("EMA5ahead",temp_day)
    full_results[,temp_colname] <- ((full_results[,temp_EMACol] / full_results[,"avgOpenClose"]) > temp_pctVal)
  }
  temp_colname <- paste0("outcomes_",temp_day,"d","0pctEMA5below")
  temp_EMACol <- paste0("EMA5ahead",temp_day)
  full_results[,temp_colname] <- (full_results[,"avgOpenClose"] > full_results[,temp_EMACol])
}

saveRDS(full_results,file="E:/Market_Data/SP500_3yr_price_history_with_EMAs_and_outcomes_current_working.RDS")

full_results <- readRDS("E:/Market_Data/SP500_3yr_price_history_with_EMAs_and_outcomes_current_working.RDS")
...
option_aggregate <- readRDS(paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/full_option_aggregate_2024-06-11_to_",
                                   test_date,".RDS"))
option_aggregate <- option_aggregate[option_aggregate$option_underlying_ticker %in% SP500_tickers,]
option_aggregate <- option_aggregate[option_aggregate$option_quote_date >= "2024-06-11" & option_aggregate$option_quote_date <= test_date,]
temp_aggregate <- readRDS("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/full_option_aggregate_2024-01-22_to_2024-06-10.RDS")
temp_aggregate <- temp_aggregate[temp_aggregate$option_underlying_ticker %in% SP500_tickers,]
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2024-01-22" & temp_aggregate$option_quote_date <= "2024-06-10",]
...
option_aggregate <- as.data.frame(rbind(temp_aggregate,option_aggregate))
temp_rowCheck <- paste0(option_aggregate$option_underlying_ticker,"_",
                        format(option_aggregate$option_quote_date, format = "%Y%m%d"))
option_aggregate <- option_aggregate[!duplicated(temp_rowCheck),]
option_aggregate <- option_aggregate[order(option_aggregate$option_underlying_ticker,option_aggregate$option_quote_date),]

EMA5_aggregate <- readRDS(paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA5_aggregate_2024-06-11_to_",
                                 test_date,".RDS"))
EMA5_aggregate <- EMA5_aggregate[EMA5_aggregate$option_underlying_ticker %in% SP500_tickers,]
EMA5_aggregate <- EMA5_aggregate[EMA5_aggregate$option_quote_date >= "2024-06-11" & EMA5_aggregate$option_quote_date <= test_date,]
temp_aggregate <- readRDS("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA5_aggregate_2024-01-22_to_2024-06-10.RDS")
temp_aggregate <- temp_aggregate[temp_aggregate$option_underlying_ticker %in% SP500_tickers,]
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2024-01-22" & temp_aggregate$option_quote_date <= "2024-06-10",]
...
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2021-05-14" & temp_aggregate$option_quote_date <= "2021-09-30",]
EMA5_aggregate <- as.data.frame(rbind(temp_aggregate,EMA5_aggregate))
temp_rowCheck <- paste0(EMA5_aggregate$option_underlying_ticker,"_",
                        format(EMA5_aggregate$option_quote_date, format = "%Y%m%d"))
EMA5_aggregate <- EMA5_aggregate[!duplicated(temp_rowCheck),]
EMA5_aggregate <- EMA5_aggregate[order(EMA5_aggregate$option_underlying_ticker,EMA5_aggregate$option_quote_date),]

EMA10_aggregate <- readRDS(paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA10_aggregate_2024-06-11_to_",
                                  test_date,".RDS"))
EMA10_aggregate <- EMA10_aggregate[EMA10_aggregate$option_underlying_ticker %in% SP500_tickers,]
EMA10_aggregate <- EMA10_aggregate[EMA10_aggregate$option_quote_date >= "2024-06-11" & EMA10_aggregate$option_quote_date <= test_date,]
temp_aggregate <- readRDS("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA10_aggregate_2024-01-22_to_2024-06-10.RDS")
temp_aggregate <- temp_aggregate[temp_aggregate$option_underlying_ticker %in% SP500_tickers,]
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2024-01-22" & temp_aggregate$option_quote_date <= "2024-06-10",]
...
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2021-05-14" & temp_aggregate$option_quote_date <= "2021-09-30",]
EMA10_aggregate <- as.data.frame(rbind(temp_aggregate,EMA10_aggregate))
temp_rowCheck <- paste0(EMA10_aggregate$option_underlying_ticker,"_",
                        format(EMA10_aggregate$option_quote_date, format = "%Y%m%d"))
EMA10_aggregate <- EMA10_aggregate[!duplicated(temp_rowCheck),]
EMA10_aggregate <- EMA10_aggregate[order(EMA10_aggregate$option_underlying_ticker,EMA10_aggregate$option_quote_date),]

EMA20_aggregate <- readRDS(paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA20_aggregate_2024-06-11_to_",
                                  test_date,".RDS"))
EMA20_aggregate <- EMA20_aggregate[EMA20_aggregate$option_underlying_ticker %in% SP500_tickers,]
EMA20_aggregate <- EMA20_aggregate[EMA20_aggregate$option_quote_date >= "2024-06-11" & EMA20_aggregate$option_quote_date <= test_date,]
temp_aggregate <- readRDS("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA20_aggregate_2024-01-22_to_2024-06-10.RDS")
temp_aggregate <- temp_aggregate[temp_aggregate$option_underlying_ticker %in% SP500_tickers,]
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2024-01-22" & temp_aggregate$option_quote_date <= "2024-06-10",]
...
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2021-05-14" & temp_aggregate$option_quote_date <= "2021-09-30",]
EMA20_aggregate <- as.data.frame(rbind(temp_aggregate,EMA20_aggregate))
temp_rowCheck <- paste0(EMA20_aggregate$option_underlying_ticker,"_",
                        format(EMA20_aggregate$option_quote_date, format = "%Y%m%d"))
EMA20_aggregate <- EMA20_aggregate[!duplicated(temp_rowCheck),]
EMA20_aggregate <- EMA20_aggregate[order(EMA20_aggregate$option_underlying_ticker,EMA20_aggregate$option_quote_date),]

EMA50_aggregate <- readRDS(paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA50_aggregate_2024-06-11_to_",
                                  test_date,".RDS"))
EMA50_aggregate <- EMA50_aggregate[EMA50_aggregate$option_underlying_ticker %in% SP500_tickers,]
EMA50_aggregate <- EMA50_aggregate[EMA50_aggregate$option_quote_date >= "2024-06-11" & EMA50_aggregate$option_quote_date <= test_date,]
temp_aggregate <- readRDS("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/EMA50_aggregate_2024-01-22_to_2024-06-10.RDS")
temp_aggregate <- temp_aggregate[temp_aggregate$option_underlying_ticker %in% SP500_tickers,]
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2024-01-22" & temp_aggregate$option_quote_date <= "2024-06-10",]
...
temp_aggregate <- temp_aggregate[temp_aggregate$option_quote_date >= "2021-05-14" & temp_aggregate$option_quote_date <= "2021-09-30",]
EMA50_aggregate <- as.data.frame(rbind(temp_aggregate,EMA50_aggregate))
temp_rowCheck <- paste0(EMA50_aggregate$option_underlying_ticker,"_",
                        format(EMA50_aggregate$option_quote_date, format = "%Y%m%d"))
EMA50_aggregate <- EMA50_aggregate[!duplicated(temp_rowCheck),]
...
SP500_splits <- readRDS(paste0("E:/Market_Data/SP500_splits_current_working.RDS"))
...
all_rates <- readRDS("E:/Market_Data/FedRateDF_current_working.RDS")
...
sub_aggregate <- match_aggregate[,!grepl("ahead",colnames(match_aggregate)) & !grepl("outcomes_",colnames(match_aggregate))]
...
write.table(t(test_input),file=paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/test_input_V4_for_prediction_with_NN_20240314_to_",
                                      paste0(unlist(str_split(test_date,"-")),collapse = ""),".tsv"),sep="\t")
saveRDS(match_aggregate,paste0("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/full_aggregate_V4_for_prediction_with_NN_20240314_to_",

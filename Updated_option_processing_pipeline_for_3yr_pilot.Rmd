...
match_aggregate <- readRDS("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/full_aggregate_V3_for_prediction_with_NN_20240314_to_20240429.RDS")

prediction_dataset <- read.csv("E:/Market_Data/DiscountOptionData/DTNSubscription/revised_derived_aggregates/Freeze20240404/realtime_testing_predictions_merged4up10d_output.tsv",sep="\t")
rownames(prediction_dataset) <- rownames(match_aggregate)
prediction_dataset <- cbind(prediction_dataset,match_aggregate[rownames(prediction_dataset),])

date_to_check <- "2024-04-15"
...
temp_pred <- prediction_dataset[prediction_dataset$EOD_option_quote_date == date_to_check,]
temp_pred <- cbind(temp_pred,temp_mbData[temp_pred$EOD_option_underlying_ticker,])
test <- temp_pred[temp_pred$Predicted == "True",]
test[1:5,c("Predicted","avgOpenClose","EOD_option_underlying_ticker","EOD_option_quote_date","relative_avgOpenClose_ahead10",
           colnames(temp_mbData))]
test_upside_val <- unlist(lapply(test$updownside,function(x){as.numeric(str_replace(str_split(x," ")[[1]][1],"%",""))}))
test_upside_dir <- unlist(lapply(test$updownside,function(x){str_split(x," ")[[1]][2]}))
test[test_upside_dir == "Upside" & test_upside_val > 40,c("Predicted","avgOpenClose","EOD_option_underlying_ticker",
                                                          "EOD_option_quote_date","relative_avgOpenClose_ahead10",colnames(temp_mbData))]
